...
def entrenar_modelo_generico(modelo, datos, epocas_solicitadas=100, batch_size=32):
    ...
    X_tr, X_val, ent_tr, ent_val, y_tr, y_val = train_test_split(
        datos.X_train, datos.entidad_ids_train, datos.y_train,
        test_size=0.15, stratify=datos.entidad_ids_train, random_state=42,
    )

    callbacks = [
        keras.callbacks.EarlyStopping(monitor="val_loss", patience=PACIENCIA_EARLY_STOPPING, restore_best_weights=True),
        keras.callbacks.ReduceLROnPlateau(monitor="val_loss", factor=0.5, patience=5, min_lr=1e-5),
    ]
    historial = modelo.fit(
        [X_tr, ent_tr.reshape(-1, 1)],
        {"P50": y_tr, "P90": y_tr, "P10": y_tr},
        validation_data=([X_val, ent_val.reshape(-1, 1)], {"P50": y_val, "P90": y_val, "P10": y_val}),
...
VERSION_ARQUITECTURA = "v3-semilla-fija"
...
def render_seccion_dataset_propio():
    ...
    with col_izq:
        ...
        st.markdown("**Confirma las columnas detectadas**")
        c1, c2 = st.columns(2)
        with c1:
        ...
        c3, c4 = st.columns(2)
        c3.metric("Desde", resumen["fecha_min"].strftime("%Y-%m-%d"))
        c4.metric("Hasta", resumen["fecha_max"].strftime("%Y-%m-%d"))
        c5, c6 = st.columns(2)
        c5.metric("Años cubiertos (aprox.)", f"{resumen['anios_aprox']:.1f}")
        c6.metric("Entidades", resumen["n_entidades"])
    ...
    col_bloque2, col_bloque3 = st.columns(2)

    with col_bloque2:
        st.subheader("Entrenamiento y validación del modelo")
    ...
    with col_bloque3:
        ...
        resultado_wi = st.session_state.get("motor_generico_resultado")
        if resultado_wi is None:
            st.info("Entrena el modelo primero (bloque de la izquierda) para poder simular un escenario.")
        else:
